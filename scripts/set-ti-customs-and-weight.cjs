// scripts/set-ti-customs-and-weight.cjs
//
// One-off script to set customs + weight fields for variants where SKU starts with:
// - "BE"  (e.g. BE-...)
// - "Ti"  (e.g. Ti-...)
//
...
// - Weight: 1 gram
//
// Only affects variants with SKU beginning "BE" or "Ti".
...
function skuMatches(variant) {
  ...
  // Case-sensitive (same style as your previous scripts)
  return sku.startsWith('BE') || sku.startsWith('Ti');

  // If you want case-insensitive, use:
  // const s = sku.toLowerCase();
  // return s.startsWith('be') || s.startsWith('ti');
}
...
async function run() {
  console.log('▶ Starting titanium customs + weight update for BE/Ti SKUs');
  ...
  console.log(`   Variants matched (BE/Ti): ${totalMatchedSku}`);

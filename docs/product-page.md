...
The `src/pages/item/[handle].tsx` page is statically generated at build time using `getStaticPaths` and `getStaticProps`. Each product handle is prebuilt and revalidated periodically so the page can be served from the CDN.
